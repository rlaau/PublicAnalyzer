...
import (
	...
	"github.com/rlaaudgjs5638/chainAnalyzer/internal/apool/rel/triplet/domain"
...
func (r *FileDepositRepository) rewriteFile(deposits []*domain.DetectedDepositWithEvidence) error {
	...
	for _, deposit := range deposits {
		record := []string{
			...
			deposit.DetectedAt.Format(chaintimer.EhtTimeFormat),
...
func (r *FileDepositRepository) parseDepositRecord(record []string) (*domain.DetectedDepositWithEvidence, error) {
	...
	// Parse detected time
	detectedAt, err := chaintimer.Parse(chaintimer.EhtTimeFormat, record[1])
	if err != nil {
