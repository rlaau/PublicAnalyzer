...
func NewTxIngester(isTesting bool, kafkaProducer kafka.Producer[domain.MarkedTransaction], cceService txingester.CCEService, consumerMonitor, producerMonitor monitoring.TPSMeter) TxIngester {

	if isTesting {
		return NewTestingIngester(kafkaProducer, cceService, consumerMonitor, producerMonitor)
	} else {
