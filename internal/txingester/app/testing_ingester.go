...
func NewTestingIngester(kafkaProducer kafka.Producer[domain.MarkedTransaction], cceService txingester.CCEService) *TestingIngester {
...
func NewTestingIngesterWithBatch(batchProducer kafka.BatchProducer[domain.MarkedTransaction], cceService txingester.CCEService, batchSize int, batchTimeout time.Duration) *TestingIngester {
	return &TestingIngester{
		state:             &IngestState{},
		minInterval:       1 * time.Second,
...
func (app *TestingIngester) IngestTransaction(ctx context.Context) error {
	...
	for {
		...
		if app.batchMode && app.batchProducer != nil {
			// 배치 전송 모드
			if err := app.publishBatch(ctx, rawTxs, markedTxs); err != nil {
...
func (app *TestingIngester) loadTestBatch(_ int, limit int) []domain.RawTransaction {
...
func (app *TestingIngester) publishBatch(ctx context.Context, rawTxs []domain.RawTransaction, markedTxs []domain.MarkedTransaction) error {
	...
	// 배치 전송
	return app.batchProducer.PublishMessagesBatch(ctx, messages)
}
