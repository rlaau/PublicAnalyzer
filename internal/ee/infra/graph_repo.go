...
type GraphRepository interface {
	...
	DeleteStaleEdges(threshold chaintimer.ChainTime) error
	...
	for _, conn := range adjData.Connections {
		if conn.ConnectedAddr == addrB.String() {
			...
			return &domain.EOAEdge{
				...
				Evidence:      evidence,
				FirstSeen:     chaintimer.Unix(conn.FirstSeen, 0),
				LastConfirmed: chaintimer.Unix(conn.LastConfirmed, 0),
			}, nil
		}
	}
...
func (r *BadgerGraphRepository) DeleteStaleNodes(threshold chaintimer.ChainTime) error {
...
func (r *BadgerGraphRepository) DeleteStaleEdges(threshold chaintimer.ChainTime) error {
	thresholdUnix := threshold.Unix()
