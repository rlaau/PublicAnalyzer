...
// SuccessCountMeterDesign는 OK/ERR/기타 식별자별 카운팅을 위한 인터페이스
type SuccessCountMeterDesign interface {
	AddErrCount()
...
// OkCountMeter는 병렬 안전한 카운팅 메터
type OkCountMeter struct {
	okCount    int64
	errCount   int64
...
// NewOkCountMeter 생성자
func NewOkCountMeter() *OkCountMeter {
	return &OkCountMeter{
...
func (m *OkCountMeter) GetAll() map[string]int64 {
	m.counterMu.RLock()
	defer m.counterMu.RUnlock()
