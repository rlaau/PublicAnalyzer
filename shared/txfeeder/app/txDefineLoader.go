...
func (loader *TxDefineLoader) generateDeterministicGraph() error {
	...
	// 2. Deposit 주소 200개 선택 (MockDepositAddrs에서)
	allDeposits := make([]sharedDomain.Address, 0, 200)
	for i := 0; i < 200; i++ {
		depositAddr := loader.mockDepositAddrs.GetRandomAddress()
...
// generateDetailedTransactions 상세한 트랜잭션 생성
func (loader *TxDefineLoader) generateDetailedTransactions(graph *DeterministicGraphStructure) {
	...
	// 1. MultiUser Deposits -> User 트랜잭션 (평균 11명 × 150개 = 1650개)
	for deposit, users := range graph.DepositToUsers {
		// MultiUser Deposit인지 확인
		isMultiUser := false
		for _, multiDeposit := range graph.MultiUserDeposits {
			if deposit == multiDeposit {
				isMultiUser = true
				break
			}
		}
		...
		if isMultiUser {
			for _, user := range users {
				tx := loader.createMarkedTransaction(deposit, user, len(transactions), currentTime)
				transactions = append(transactions, tx)
				graph.TransactionsByCategory["deposit_to_user"]++
	...
	// 2. SingleUser Deposits -> User 트랜잭션 (1명 × 50개 = 50개)
	for deposit, users := range graph.DepositToUsers {
		// SingleUser Deposit인지 확인
		isSingleUser := false
		for _, singleDeposit := range graph.SingleUserDeposits {
			if deposit == singleDeposit {
				isSingleUser = true
				break
			}
		}
		
		if isSingleUser && len(users) > 0 {
			user := users[0]
			tx := loader.createMarkedTransaction(deposit, user, len(transactions), currentTime)
			transactions = append(transactions, tx)
			graph.TransactionsByCategory["single_deposit_to_user"]++
			
			currentTime = currentTime.Add(1 * time.Minute)
		}
	}

	// 3. MultiDeposit Users -> Deposits 트랜잭션 (평균 3.5개 × 500명 = 1750개)
	for _, user := range graph.MultiDepositUsers {
		...
		for _, deposit := range deposits {
			...
			transactions = append(transactions, tx)
			graph.TransactionsByCategory["multi_user_to_deposit"]++
			
			currentTime = currentTime.Add(30 * time.Second)
		}
	}

	// 4. SingleDeposit Users -> Deposit 트랜잭션 (1개 × 300명 = 300개)
	for _, user := range graph.SingleDepositUsers {
		...
		if len(deposits) > 0 {
			...
			tx := loader.createMarkedTransaction(user, deposit, len(transactions), currentTime)
			transactions = append(transactions, tx)
			graph.TransactionsByCategory["single_user_to_deposit"]++
			
			currentTime = currentTime.Add(30 * time.Second)
		}
	}

	// 5. User간 직접 연결 트랜잭션 (200개)
	for _, pair := range graph.UserToUserPairs {
		tx := loader.createMarkedTransaction(pair[0], pair[1], len(transactions), currentTime)
		transactions = append(transactions, tx)
	...
	// 6. 일부 CEX 트랜잭션 추가 (다양성을 위해)
	for i := 0; i < 100; i++ {
		cex := graph.CEXAddresses[i%len(graph.CEXAddresses)]
		deposit := graph.DepositAddresses[i%len(graph.DepositAddresses)]
		
		tx := loader.createMarkedTransaction(cex, deposit, len(transactions), currentTime)
		transactions = append(transactions, tx)
		graph.TransactionsByCategory["cex_to_deposit"]++
...
func (loader *TxDefineLoader) sendTransactions(ctx context.Context) {
	...
	interval := 20 * time.Millisecond // 초당 50개 전송
...
func (loader *TxDefineLoader) sendTransactionsBatch(ctx context.Context) {
	...
	batchInterval := time.Duration(loader.batchSize) * 20 * time.Millisecond
